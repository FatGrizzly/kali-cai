...
RUN apt-get update && \
    apt-get install -y --no-install-recommends \
        ...
        curl wget git ca-certificates gpg \
    && apt-get clean \
    && rm -rf /var/lib/apt/lists/*
...
RUN echo '#!/bin/bash' > /opt/cai/start.sh && \
    ...
    echo 'ENV_FILE="/config/.env"' >> /opt/cai/start.sh && \
    echo '' >> /opt/cai/start.sh && \
    echo 'if [ -f "$ENV_FILE" ]; then' >> /opt/cai/start.sh && \
    echo '  echo "Loading and exporting env from $ENV_FILE"' >> /opt/cai/start.sh && \
    echo '  set -a' >> /opt/cai/start.sh && \
    echo '  source "$ENV_FILE"' >> /opt/cai/start.sh && \
    echo '  set +a' >> /opt/cai/start.sh && \
    echo 'else' >> /opt/cai/start.sh && \
    echo '  echo "No .env found at $ENV_FILE"' >> /opt/cai/start.sh && \
